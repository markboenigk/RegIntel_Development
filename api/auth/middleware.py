...
class AuthMiddleware:
    ...
    async def get_optional_user(self, request: Request) -> Optional[UserProfile]:
        ...
        try:
            ...
            # If no token in cookies, try Authorization header for local development
            if not token:
                auth_header = request.headers.get("Authorization")
                if auth_header and auth_header.startswith("Bearer "):
                    token = auth_header[7:]  # Remove "Bearer " prefix
                    print(f"🔍 Token found in Authorization header: {'Yes' if token else 'No'}")
            ...
            try:
                ...
                if user_response.user:
                    ...
                    print(f"🔐 Creating user profile from Supabase response...")
