...
# RAG Configuration
STRICT_RAG_ONLY = os.getenv("STRICT_RAG_ONLY", "true").lower() == "true"  # Only use pro vided sources, no external knowledge
...
# Include authentication routes - with CI environment handling
try:
    ...
    # In CI environment, we can continue without auth for basic tests
    if os.getenv('CI') or os.getenv('GITHUB_ACTIONS'):
        print(f"⚠️ CI environment: Authentication routes not loaded due to: {str(e)}")
        print("   Continuing without authentication for basic application tests")
        # Create dummy functions for CI
        async def get_current_user(request):
            raise HTTPException(status_code=401, detail="Authentication not available in CI")
        async def get_optional_user(request):
            return None
    else:
        # In production/development, re-raise the error
        raise
...
@app.get("/api/")
async def api_root():
    """API root endpoint for Vercel health check."""
    return {"message": "RegIntel API is running", "status": "healthy"}
...
async def login_page(request: Request):
    """Serve the login page HTML template"""
